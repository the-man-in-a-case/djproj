...
class Watcher:
    ...
    def _handle_redis_event(self, ch: str, data_str: str):
        ...
        if task_key not in self.task_status_tracker:
            self.task_status_tracker[task_key] = {"pid": pid, "task_id": task_id, "task_name": task_name, "has_redis_event": False, "has_celery_event": False}
        ...
        if status == "RECEIVED" and not self.task_status_tracker[task_key].get("status_recorded", False):
            self.project_stats[pid]["received"] += 1
            self.task_status_tracker[task_key]["status_recorded"] = True
        ...
        elif status == "STARTED" and not self.task_status_tracker[task_key].get("status_recorded", False):
        ...
        elif status == "SUCCESS" and not self.task_status_tracker[task_key].get("final_status_recorded", False):
            # 只有在任务未被标记为完成时才更新统计
            self.project_stats[pid]["succeeded"] += 1
            self.task_status_tracker[task_key]["final_status_recorded"] = True
            should_update_global = True
            # 使用指定格式显示进度
        ...
        elif status in ["ERROR", "FAILED"] and not self.task_status_tracker[task_key].get("final_status_recorded", False):
            # 只有在任务未被标记为完成时才更新统计
    ...
    def _handle_celery_event(self, e: Dict[str, Any]):
        ...
        if task_project_id:
            ...
            for task_key, task_info in self.task_status_tracker.items():
                if task_info["pid"] == task_project_id:
                    # 可以根据更多条件进行匹配，这里简单处理
    ...
    def run(self):
        ...
        print(f"Global task summary: received={total_received} started={total_started} succeeded={total_succeeded} failed={total_failed}")
        
        # 显示每个项目的详细统计
        for pid, stats in self.project_stats.items():
            # 计算这个项目的完成率
            received = stats["received"]
            if received > 0:
                completion_rate = (stats["succeeded"] + stats["failed"]) / received * 100
            else:
                completion_rate = 0
            
            print(f"Project {short(pid)}: received={stats['received']} started={stats['started']} succeeded={stats['succeeded']} failed={stats['failed']} ({completion_rate:.1f}% complete)")
